...
async function createOrder(db, body) {
  ...
  const shippingThreshold = Math.max(0, Number(settings.freeShippingThreshold || 25000));
  const shipping = subtotal >= shippingThreshold || activeCoupon?.type === 'shipping' ? 0 : Math.max(0, Number(settings.flatShippingRate || 0));
