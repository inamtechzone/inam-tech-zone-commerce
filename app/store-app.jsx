...
function Hero({ settings, featured, onShop, onProduct, onQuote }) {
  ...
  return <section className="hero" aria-labelledby="hero-title">
    ...
    <div className="hero-visual">
      <img src={settings.heroImage} alt={heroProduct.name} />
...
function ProductCard({ product, settings, onProduct, onAdd, wished, onWish, onCompare, compared }) {
  return <article className="product-card">
    <div className="product-card-image">
      ...
      <button className="image-button" onClick={() => onProduct(product)}><img src={product.image} alt={product.name} /></button>
...
function CartDrawer({ open, cart, products, settings, onClose, onQty, onRemove, onCheckout }) {
  ...
  return <><div className={`drawer-scrim ${open ? 'show' : ''}`} onClick={onClose} /><aside className={`cart-drawer ${open ? 'open' : ''}`} aria-hidden={!open}>
    ...
    <div className="cart-lines">{lines.length ? lines.map(({ product, qty }) => <div className="cart-line" key={product.id}><img src={product.image} alt="" /><div><p>{product.category}</p><h3>{product.name}</h3><span>{product.color}</span><div className="mini-stepper"><button onClick={() => onQty(product.id, -1)}>−</button><b>{qty}</b><button onClick={() => onQty(product.id, 1)}>＋</button></div></div><div className="cart-line-end"><button onClick={() => onRemove(product.id)}>Remove</button><strong>{formatMoney(product.price * qty, settings)}</strong></div></div>) : <div className="empty-cart"><span>○</span><h3>Your bag is waiting</h3><p>Start with something designed to stay.</p><button className="button button-dark" onClick={onClose}>Continue shopping</button></div>}</div>
...
function CheckoutView({ cart, products, settings, couponList, onBack, onPlaceOrder }) {
  ...
  const submit = async (e) => { e.preventDefault(); setSubmitting(true); const fields = Object.fromEntries(new FormData(e.currentTarget)); await onPlaceOrder({ customer: fields, payment, lines, subtotal, discount, shipping, tax, total, coupon: applied?.code || '' }); setSubmitting(false); };
...
const adminNav = [
  ['Overview', '◫'], ['Products', '◇'], ['Orders', '▤'], ['Quotes', '◈'], ['Categories', '⌘'], ['Customers', '◎'], ['Promotions', '%'], ['Analytics', '↗'], ['Settings', '⚙'],
];
...
function AdminShell({ settings, section, setSection, onStore, onLogout, orderCount, syncState, onRefresh, children }) {
  ...
  return <div className="admin-app">
    ...
    <div className="admin-main"><header className="admin-header"><button className="admin-menu" onClick={() => setOpen(!open)}>☰</button><div><p>Workspace / {section}</p><h1>{section}</h1></div><div className="admin-header-actions"><div className={`sync-indicator ${syncState.status}`}><i /><span><strong>{syncState.status === 'syncing' ? 'Synchronizing' : syncState.status === 'error' ? 'Sync issue' : syncState.status === 'local' ? 'Device only' : 'Google synced'}</strong><small>{syncState.message}</small></span><button type="button" onClick={onRefresh} aria-label="Refresh shared data" title="Refresh shared data">↻</button></div><button onClick={onStore}>View store ↗</button></div></header>{children}</div>
  </div>;
}
...
function ProductEditor({ product, categories, onClose, onSave, onUpload, settings }) {
  ...
  const imageFiles = async (e) => {
    const current = productImages(draft);
    const files = Array.from(e.target.files || []).slice(0, Math.max(0, 6 - current.length));
    if (!files.length) { e.target.value = ''; return; }
    setUploading(true); setImageError('');
    try {
      const uploaded = [];
      for (const file of files) uploaded.push(await onUpload(file));
      updateImages([...current, ...uploaded].slice(0, 6));
  ...
  const addImageUrl = () => {
    ...
    updateImages([...images, url]); setImageUrl(''); setImageError('');
  };
  ...
  const images = productImages(draft);
  return <div className="overlay editor-overlay" onMouseDown={onClose}><form className="product-editor" onMouseDown={(e) => e.stopPropagation()} onSubmit={submit}><div className="editor-head"><div><p className="eyebrow">Technical catalog editor</p><h2>{product ? 'Edit product' : 'New product'}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="editor-scroll"><div className="editor-grid"><section><h3>Product information</h3><label>Product title<input required value={draft.name} onChange={(e) => set('name', e.target.value)} /></label><label>Description<textarea rows="5" value={draft.description} onChange={(e) => set('description', e.target.value)} /></label><div className="form-grid"><label>Category<select value={draft.category} onChange={(e) => set('category', e.target.value)}>{categories.map((c) => <option key={c.id}>{c.name}</option>)}</select></label><label>Configuration / color<input value={draft.color} onChange={(e) => set('color', e.target.value)} /></label><label>Brand<input value={draft.brand || ''} onChange={(e) => set('brand', e.target.value)} /></label><label>Model<input value={draft.model || ''} onChange={(e) => set('model', e.target.value)} /></label><label>Price ({settings.currency})<input required type="number" min="0" value={draft.price} onChange={(e) => set('price', e.target.value)} /></label><label>Compare-at price<input type="number" min="0" value={draft.comparePrice} onChange={(e) => set('comparePrice', e.target.value)} /></label><label>SKU<input required value={draft.sku} onChange={(e) => set('sku', e.target.value)} /></label><label>Inventory<input required type="number" min="0" value={draft.stock} onChange={(e) => set('stock', e.target.value)} /></label><label>Warranty<input value={draft.warranty || ''} onChange={(e) => set('warranty', e.target.value)} /></label><label>Lead time<input value={draft.leadTime || ''} onChange={(e) => set('leadTime', e.target.value)} /></label></div><label>Technical specifications (one per line)<textarea rows="5" value={Array.isArray(draft.details) ? draft.details.join('\n') : draft.details} onChange={(e) => set('details', e.target.value)} /></label></section><section><h3>Media & visibility</h3><div className="media-counter"><b>Product images</b><span>{images.length} / 6</span></div><label className={`image-upload multi-image-upload ${images.length >= 6 ? 'disabled' : ''}`}><div><span>{uploading ? '…' : '＋'}</span><p>{uploading ? 'Uploading images…' : images.length ? 'Add more images' : 'Upload product images'}</p><small>Minimum 1 · maximum 6</small></div><input type="file" accept="image/*" multiple disabled={uploading || images.length >= 6} onChange={imageFiles} /></label>{images.length > 0 && <div className="multi-image-grid">{images.map((image, index) => <article className={index === 0 ? 'primary' : ''} key={`${image}-${index}`}><img src={image} alt={`Product ${index + 1}`} /><span>{index === 0 ? 'Primary' : `Image ${index + 1}`}</span>{index > 0 && <button type="button" className="make-primary" onClick={() => makePrimary(image)}>Set primary</button>}<button type="button" className="remove-media" onClick={() => removeImage(image)} aria-label={`Remove image ${index + 1}`}>×</button></article>)}</div>}<div className="image-url-row"><input value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} placeholder="Paste image or Google Drive URL" disabled={images.length >= 6} /><button type="button" onClick={addImageUrl} disabled={!imageUrl.trim() || images.length >= 6}>Add</button></div>{imageError && <p className="media-error">{imageError}</p>}<label>Status<select value={draft.status} onChange={(e) => set('status', e.target.value)}><option value="active">Active</option><option value="draft">Draft</option><option value="archived">Archived</option></select></label><label>Badge<input value={draft.badge} onChange={(e) => set('badge', e.target.value)} placeholder="New, Bestseller…" /></label><label className="switch-label"><span><b>Featured product</b><small>Show this product in curated areas.</small></span><input type="checkbox" checked={draft.featured} onChange={(e) => set('featured', e.target.checked)} /></label></section></div></div><div className="editor-actions"><button type="button" onClick={onClose}>Cancel</button><button className="admin-primary" disabled={uploading || saving}>{saving ? 'Saving & syncing…' : 'Save product'}</button></div></form></div>;
}

function AdminProductPreview({ product, settings, onClose, onEdit }) {
  if (!product) return null;
  return <div className="overlay admin-preview-overlay" onMouseDown={onClose}><section className="admin-product-preview" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose}>×</button><div className="admin-preview-image"><img src={product.image} alt={product.name} /><span>{product.badge || product.status}</span></div><div className="admin-preview-copy"><p className="eyebrow">Storefront preview · {product.category}</p><h2>{product.name}</h2><div className="admin-preview-meta"><span>SKU<strong>{product.sku}</strong></span><span>Brand / model<strong>{product.brand || '—'} · {product.model || '—'}</strong></span><span>Stock<strong>{product.stock} units</strong></span><span>Status<strong>{product.status}</strong></span></div><div className="detail-price">{formatMoney(product.price, settings)} {product.comparePrice > product.price && <del>{formatMoney(product.comparePrice, settings)}</del>}</div><p>{product.description}</p><ul>{(product.details || []).map((detail) => <li key={detail}>{detail}</li>)}</ul><div className="admin-preview-actions"><button onClick={onClose}>Close preview</button><button className="admin-primary" onClick={() => { onClose(); onEdit(product); }}>Edit product</button></div></div></section></div>;
}
...
function AdminSettings({ settings, setSettings, onSave }) {
  ...
  const save = async () => { setSaving(true); try { await onSave(draft); setSettings(draft); } catch {} finally { setSaving(false); } };
  return <div className="admin-content"><div className="settings-layout"><aside className="settings-nav">{tabs.map((name) => <button className={tab === name ? 'active' : ''} key={name} onClick={() => setTab(name)}>{name}<span>→</span></button>)}</aside><section className="admin-card settings-card"><div className="settings-head"><div><p className="eyebrow">Store configuration</p><h2>{tab}</h2><p>Changes update the storefront immediately and sync to Google Sheets when connected.</p></div><button className="admin-primary" onClick={save} disabled={saving}>{saving ? 'Saving…' : 'Save changes'}</button></div>
    ...
    {tab === 'Integrations' && <div className="settings-section"><h3>Google workspace backend</h3><div className="integration-note"><span>G</span><div><b>Google Apps Script + Sheets + Drive</b><p>Every device connects through one shared application gateway, so PC, mobile and laptop automatically use the same Google Sheet.</p></div></div><div className="form-grid"><label className="full">Shared synchronization gateway<input value="Connected → Google Apps Script" readOnly /><small className="managed-setting">The verified shared endpoint is included in this website—no device-level setup is required.</small></label><label className="full">Google Drive folder ID<input value={draft.driveFolderId} onChange={(e) => set('driveFolderId', e.target.value)} placeholder="Created automatically by setupStore()" /></label><label>Administrator email<input type="email" value={draft.adminEmail} onChange={(e) => set('adminEmail', e.target.value)} /></label></div><div className="platform-grid"><div><b>GitHub</b><span>Workflow ready</span><small>Version control + automated build</small></div><div><b>Shared API</b><span>Gateway active</span><small>One connection for every device</small></div><div><b>Google Sheets</b><span>Authoritative source</span><small>Products and records synchronized</small></div></div></div>}
  </section></div></div>;
}
...
function AdminSimple({ section, categories, setCategories, customers, setCustomers, coupons, setCoupons, orders, setOrders, quotes, setQuotes, settings, sync, onOrderSelect, onOrderDelete }) {
  ...
  if (section === 'Orders') return <div className="admin-content"><section className="admin-card"><div className="admin-card-head"><div><h3>All orders</h3><p>Preview every order, update fulfillment and manage shipment tracking.</p></div><span className="quote-count">{orders.filter((item) => !['Delivered', 'Cancelled'].includes(item.status)).length} open</span></div><OrderTable orders={orders} settings={settings} onSelect={onOrderSelect} onDelete={onOrderDelete} onStatus={(id, status) => commit('updateOrder', { orderId: id, status }, () => setOrders(orders.map((order) => order.id === id ? { ...order, status } : order)), `Order ${id} status saved`)} /></section></div>;
  if (section === 'Quotes') return <div className="admin-content"><section className="admin-card"><div className="admin-card-head"><div><h3>Project quote requests</h3><p>Track commercial inquiries from first contact to proposal.</p></div><span className="quote-count">{quotes.filter((item) => item.status === 'New').length} new</span></div><div className="table-wrap"><table><thead><tr><th>Request</th><th>Customer</th><th>Project</th><th>Product / system</th><th>Qty</th><th>Status</th><th>Action</th></tr></thead><tbody>{quotes.map((quote) => <tr key={quote.id}><td><strong>{quote.id}</strong><small className="table-subline">{quote.date}</small></td><td><div className="customer-cell"><span>{quote.customer.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span><div><strong>{quote.customer}</strong><small>{quote.company || quote.email} · {quote.phone}</small></div></div></td><td>{quote.projectType}</td><td>{quote.product || quote.productLabel || 'Complete system'}</td><td>{quote.quantity || 1}</td><td><select className={`status ${String(quote.status).toLowerCase()}`} value={quote.status} onChange={(event) => { const status = event.target.value; commit('updateQuote', { quoteId: quote.id, status }, () => setQuotes(quotes.map((item) => item.id === quote.id ? { ...item, status } : item)), `Quote ${quote.id} saved successfully`); }}><option>New</option><option>Contacted</option><option>Quoted</option><option>Won</option><option>Closed</option></select></td><td><button className="table-delete-button" onClick={() => { if (!confirm(`Delete quote ${quote.id}?`)) return; commit('deleteQuote', { quoteId: quote.id }, () => setQuotes(quotes.filter((item) => item.id !== quote.id)), `Quote ${quote.id} deleted`); }}>Delete</button></td></tr>)}</tbody></table></div></section></div>;
  if (section === 'Categories') return <div className="admin-content"><section className="admin-card list-card"><div className="admin-card-head"><div><h3>Product categories</h3><p>Control the storefront&apos;s shop navigation.</p></div><button className="admin-primary" onClick={() => { const category = { id: uid('CAT'), name: 'New category', slug: `new-category-${categories.length + 1}`, active: true }; commit('saveCategory', { category }, () => setCategories([...categories, category]), 'Category created and synchronized'); }}>＋ Add category</button></div>{categories.map((category, index) => <div className="editable-row" key={category.id}><span>{String(index + 1).padStart(2, '0')}</span><input value={category.name} onChange={(event) => setCategories(categories.map((item) => item.id === category.id ? { ...item, name: event.target.value, slug: event.target.value.toLowerCase().replace(/\W+/g, '-') } : item))} /><small>/{category.slug}</small><label className="mini-switch"><input type="checkbox" checked={category.active} onChange={(event) => { const updated = { ...category, active: event.target.checked }; commit('saveCategory', { category: updated }, () => setCategories(categories.map((item) => item.id === category.id ? updated : item)), `${category.name} visibility saved`); }} /></label><div className="record-actions"><button className="record-save-button" onClick={() => commit('saveCategory', { category }, null, `${category.name} saved successfully`)}>Save</button><button className="record-delete-button" title="Delete category" aria-label={`Delete ${category.name}`} onClick={() => { if (!confirm(`Delete category “${category.name}”?`)) return; commit('deleteCategory', { categoryId: category.id }, () => setCategories(categories.filter((item) => item.id !== category.id)), `${category.name} deleted`); }}>Delete</button></div></div>)}</section></div>;
  if (section === 'Customers') return <div className="admin-content"><section className="admin-card"><div className="admin-card-head"><div><h3>Customers</h3><p>{customers.length} active customer profiles</p></div><span className="live-data-label">SHEET DATA</span></div><div className="table-wrap"><table><thead><tr><th>Customer</th><th>Joined</th><th>Orders</th><th>Total spent</th><th>Segment</th><th>Action</th></tr></thead><tbody>{customers.map((customer) => <tr key={customer.id}><td><div className="customer-cell"><span>{customer.name.split(' ').map((x) => x[0]).join('')}</span><div><strong>{customer.name}</strong><small>{customer.email}</small></div></div></td><td>{customer.joined}</td><td>{customer.orders}</td><td><strong>{formatMoney(customer.spent, settings)}</strong></td><td><span className="status active">{customer.spent > 1000 ? 'VIP' : 'Returning'}</span></td><td><button className="table-delete-button" onClick={() => { if (!confirm(`Delete customer ${customer.name}?`)) return; commit('deleteCustomer', { customerId: customer.id }, () => setCustomers(customers.filter((item) => item.id !== customer.id)), `${customer.name} deleted`); }}>Delete</button></td></tr>)}</tbody></table></div></section></div>;
  if (section === 'Promotions') return <div className="admin-content"><section className="admin-card list-card"><div className="admin-card-head"><div><h3>Discount codes</h3><p>Codes are validated instantly at checkout.</p></div><button className="admin-primary" onClick={() => { const coupon = { code: `ITZ${Date.now().toString(36).slice(-5).toUpperCase()}`, type: 'percent', value: 10, active: true, uses: 0 }; commit('saveCoupon', { coupon }, () => setCoupons([...coupons, coupon]), 'Promotion created and synchronized'); }}>＋ Create discount</button></div>{coupons.map((coupon) => <div className="coupon-row" key={coupon.code}><strong>{coupon.code}</strong><select value={coupon.type} onChange={(event) => setCoupons(coupons.map((item) => item.code === coupon.code ? { ...item, type: event.target.value } : item))}><option value="percent">Percentage</option><option value="fixed">Fixed amount</option><option value="shipping">Free shipping</option></select><input type="number" value={coupon.value} onChange={(event) => setCoupons(coupons.map((item) => item.code === coupon.code ? { ...item, value: Number(event.target.value) } : item))} /><span>{coupon.uses} uses</span><label className="mini-switch"><input type="checkbox" checked={coupon.active} onChange={(event) => { const updated = { ...coupon, active: event.target.checked }; commit('saveCoupon', { coupon: updated }, () => setCoupons(coupons.map((item) => item.code === coupon.code ? updated : item)), `${coupon.code} availability saved`); }} /></label><div className="record-actions"><button className="record-save-button" onClick={() => commit('saveCoupon', { coupon }, null, `${coupon.code} saved successfully`)}>Save</button><button className="record-delete-button" title="Delete promotion" aria-label={`Delete ${coupon.code}`} onClick={() => { if (!confirm(`Delete promotion ${coupon.code}?`)) return; commit('deleteCoupon', { code: coupon.code }, () => setCoupons(coupons.filter((item) => item.code !== coupon.code)), `${coupon.code} deleted`); }}>Delete</button></div></div>)}</section></div>;
...
function AdminApp({ products, setProducts, orders, setOrders, quotes, setQuotes, categories, setCategories, customers, setCustomers, coupons, setCoupons, settings, setSettings, token, setToken, onStore, toast, syncState, onSyncState, onRefresh }) {
  ...
  const sync = async (action, payload, successMessage = 'Changes saved') => {
    if (!settings.apiEndpoint) { onSyncState({ status: 'local', message: 'Google backend not connected' }); toast(`${successMessage}. Connect Google Apps Script to synchronize every device.`, 'warning', 'Saved on this device'); return { local: true }; }
    ...
    try {
      ...
      onSyncState({ status: 'error', message: error.message || 'Could not save to Google Sheets' });
      toast(error.message || 'The server did not accept this change.', 'error', 'Synchronization failed');
      throw error;
    }
  };
  const saveProduct = async (draft) => { const exists = products.some((product) => product.id === draft.id); await sync('saveProduct', { product: draft }, exists ? 'Product updated successfully' : 'Product created successfully'); setProducts(exists ? products.map((product) => product.id === draft.id ? draft : product) : [draft, ...products]); setEditor(null); };
  const deleteProduct = async (id) => { if (!confirm('Delete this product from every connected device?')) return; try { await sync('deleteProduct', { productId: id }, 'Product deleted successfully'); setProducts(products.filter((product) => product.id !== id)); setProductPreview(null); } catch {} };
  const deleteOrder = async (id) => { if (!confirm(`Delete order ${id}? This cannot be undone.`)) return; const deletedOrder = orders.find((item) => item.id === id); try { await sync('deleteOrder', { orderId: id }, `Order ${id} deleted successfully`); setOrders(orders.filter((item) => item.id !== id)); if (deletedOrder?.email) setCustomers(customers.map((customer) => customer.email?.toLowerCase() === deletedOrder.email.toLowerCase() ? { ...customer, orders: Math.max(0, Number(customer.orders || 0) - 1), spent: Math.max(0, Number(customer.spent || 0) - Number(deletedOrder.total || 0)) } : customer)); setOrderPreview(null); } catch {} };
  ...
  const upload = async (file) => {
    ...
    if (!settings.apiEndpoint) { toast('Image stored on this device only. Connect Google Drive for shared media.', 'warning', 'Local image'); return dataUrl; }
    try { const result = await apiCall(settings.apiEndpoint, 'uploadImage', { fileName: file.name, dataUrl, mimeType: file.type }, token); toast(`${file.name} uploaded to Google Drive`, 'success', 'Image uploaded'); return result.url; }
    catch (error) { toast(error.message || 'Image upload failed.', 'error', 'Upload failed'); throw error; }
  };
  ...
  if (section === 'Products') content = <AdminProducts products={products} settings={settings} onAdd={() => setEditor('new')} onEdit={setEditor} onDelete={deleteProduct} onDuplicate={duplicateProduct} onPreview={setProductPreview} />;
  ...
  if (section === 'Settings') content = <AdminSettings settings={settings} setSettings={setSettings} onSave={saveSettings} />;
  return <AdminShell settings={settings} section={section} setSection={setSection} onStore={onStore} onLogout={() => setToken('')} orderCount={orders.filter((order) => !['Delivered', 'Cancelled'].includes(order.status)).length} syncState={syncState} onRefresh={onRefresh}>{content}{editor && <ProductEditor product={editor === 'new' ? null : editor} categories={categories} settings={settings} onClose={() => setEditor(null)} onSave={saveProduct} onUpload={upload} />}{productPreview && <AdminProductPreview product={productPreview} settings={settings} onClose={() => setProductPreview(null)} onEdit={setEditor} />}{orderPreview && <OrderDetailPanel key={`${orderPreview.id}-${orderPreview.updatedAt || ''}`} order={orderPreview} settings={settings} onClose={() => setOrderPreview(null)} onSave={saveOrderDetails} onDelete={deleteOrder} />}</AdminShell>;
}

export default function StoreApp({ initialMode = 'store', initialView = 'home' }) {
  const [products, setProducts] = useStoredState('products', productSeed);
  const [orders, setOrders] = useStoredState('orders', orderSeed);
  const [categories, setCategories] = useStoredState('categories', categoriesSeed);
  const [customers, setCustomers] = useStoredState('customers', customerSeed);
  ...
  const [token, setToken] = useStoredState('admin-token', '');
  ...
  }, [mode, refreshStore, setCategories, setCoupons, setCustomers, setOrders, setProducts, setQuotes, setSettings, settings.apiEndpoint, toast, token]);
  useEffect(() => {
  ...
  const placeOrder = async (data) => {
    ...
    } catch (error) { toast(`Order is saved on this device, but server sync failed: ${error.message}`, 'error', 'Order not synchronized'); }
    setOrders((current) => [savedOrder, ...current]); setProducts(products.map((product) => { const line = data.lines.find((item) => item.id === product.id); return line ? { ...product, stock: Math.max(0, product.stock - line.qty) } : product; })); setCart([]);
    setPlacedOrder(savedOrder); setView('success'); window.scrollTo(0, 0);
  };
  const login = async ({ email, password }) => { if (settings.apiEndpoint) { const result = await apiCall(settings.apiEndpoint, 'login', { email, password }); setToken(result.token); toast('Secure Google workspace connection established.', 'success', 'Signed in successfully'); } else { if (email !== settings.adminEmail || password !== 'inamtech2026') throw new Error('Email or password is incorrect.'); setToken(`local-${Date.now()}`); toast('Admin opened in device-only mode. Connect Google Apps Script for shared data.', 'warning', 'Local administration'); } };
  ...
  const theme = {
    ...
    '--tech-blue': '#126bff',
    '--brand-violet': '#6331e6',
    '--brand-electric': '#0b74ff',
    '--brand-gradient': 'linear-gradient(135deg,#0878ff 0%,#194eea 48%,#6228dd 100%)',
