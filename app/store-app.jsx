...
function StoreImage({ src, alt = '', className = '', decoding = 'async', ...props }) {
  ...
  return <img {...props} src={currentSource} alt={alt} decoding={decoding} referrerPolicy="no-referrer" className={`${className} store-image${fallback ? ' store-image-fallback' : ''}`.trim()} onError={() => setCandidateIndex((index) => Math.min(index + 1, candidates.length - 1))} />;
}
...
function CartDrawer({ open, cart, products, settings, onClose, onQty, onRemove, onCheckout }) {
  ...
  return <><div className={`drawer-scrim ${open ? 'show' : ''}`} onClick={onClose} /><aside className={`cart-drawer ${open ? 'open' : ''}`} aria-hidden={!open}>
    ...
    {lines.length > 0 && <div className="cart-summary"><div><span>Subtotal</span><strong>{formatMoney(subtotal, settings)}</strong></div><p>Taxes and shipping calculated at checkout.</p><button className="button button-dark" onClick={onCheckout}>Secure checkout <span>→</span></button><small>Encrypted payments · Easy returns · Human support</small></div>}
  </aside></>;
}
...
function CheckoutView({ cart, products, settings, couponList, onBack, onPlaceOrder }) {
  ...
  const shipping = subtotal >= settings.freeShippingThreshold || applied?.type === 'shipping' ? 0 : settings.flatShippingRate;
...
function AdminProductPreview({ product, settings, onClose, onEdit }) {
  if (!product) return null;
  return <div className="overlay admin-preview-overlay" onMouseDown={onClose}><section className="admin-product-preview" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose}>×</button><div className="admin-preview-image"><StoreImage src={product.image} alt={product.name} /><span>{product.badge || product.status}</span></div><div className="admin-preview-copy"><p className="eyebrow">Storefront preview · {product.category}</p><h2>{product.name}</h2><div className="admin-preview-meta"><span>SKU<strong>{product.sku}</strong></span><span>Brand / model<strong>{product.brand || '—'} · {product.model || '—'}</strong></span><span>Stock<strong>{product.stock} units</strong></span><span>Status<strong>{product.status}</strong></span></div><div className="detail-price">{formatMoney(product.price, settings)} {product.comparePrice > product.price && <del>{formatMoney(product.comparePrice, settings)}</del>}</div><p>{product.description}</p><ul>{(product.details || []).map((detail) => <li key={detail}>{detail}</li>)}</ul><div className="admin-preview-actions"><button onClick={onClose}>Close preview</button><button className="admin-primary" onClick={() => { onClose(); onEdit(product); }}>Edit product</button></div></div></section></div>;
}
...
function AdminApp({ products, setProducts, orders, setOrders, quotes, setQuotes, categories, setCategories, customers, setCustomers, coupons, setCoupons, settings, setSettings, token, setToken, onStore, toast, syncState, onSyncState, onRefresh }) {
  ...
  return <AdminShell settings={settings} section={section} setSection={setSection} onStore={onStore} onLogout={() => setToken('')} orderCount={orders.filter((order) => !['Delivered', 'Cancelled'].includes(order.status)).length} syncState={syncState} onRefresh={onRefresh}>{content}{editor && <ProductEditor product={editor === 'new' ? null : editor} categories={categories} settings={settings} onClose={() => setEditor(null)} onSave={saveProduct} onUpload={upload} onImport={importImage} />}{productPreview && <AdminProductPreview product={productPreview} settings={settings} onClose={() => setProductPreview(null)} onEdit={setEditor} />}{orderPreview && <OrderDetailPanel key={`${orderPreview.id}-${orderPreview.updatedAt || ''}`} order={orderPreview} settings={settings} onClose={() => setOrderPreview(null)} onSave={saveOrderDetails} onDelete={deleteOrder} />}</AdminShell>;
}
...
export default function StoreApp({ initialMode = 'store', initialView = 'home' }) {
  ...
  useEffect(() => {
    if (mode === 'admin' || !persistentReady) return undefined;
  ...
  useEffect(() => {
    if (mode !== 'admin' || !token || !settings.apiEndpoint || !persistentReady) return undefined;
  ...
  useEffect(() => {
    if (mode !== 'admin' || token || !persistentReady) return undefined;
  ...
  const theme = {
    '--ink': isDark ? '#eef4fb' : (settings.primaryColor || '#0b1628'),
    '--lime': '#2457d6',
    '--paper': isDark ? '#080f1b' : (settings.backgroundColor || '#f3f5f7'),
    '--tech-navy': isDark ? '#0d142b' : '#111831',
    '--tech-deep': isDark ? '#050712' : '#080b18',
    '--tech-yellow': '#2457d6',
    '--tech-blue': '#2457d6',
    '--brand-violet': '#5639c7',
    '--brand-electric': '#2457d6',
    '--brand-gradient': '#2457d6',
